...
public class Zl_HosActivity extends Activity {
	...
	private HospitalChargeRes tempHosCharge;//
	private ListView lv_items, reportlist,lv_zy_fee;
	...
	private TextView tv_rq,tv_topTitle,tv_zy_time,tv_zy_totle;
	private PopupWindow pop;
	private LinearLayout lay_reportlist;
	private LinearLayout ll_header,ll_zy_fee;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.zl_zhuyuan_info);
		preferences = getSharedPreferences(PreferencesService.SPF,
				Context.MODE_PRIVATE);
		mInflater = LayoutInflater.from(Zl_HosActivity.this);
		init();
		setListener();
	}
	...
	private void init() {
		...
		ll_zy_fee=(LinearLayout)findViewById(R.id.ll_zy_fee);
		lv_zy_fee=(ListView)findViewById(R.id.lv_zy_fee);
		tv_zy_time=(TextView)findViewById(R.id.tv_zy_time);
		tv_zy_totle=(TextView)findViewById(R.id.tv_zy_totle);
		...
		rg_menu.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				case R.id.rb_cf:
					...
					ll_zy_fee.setVisibility(View.GONE);
					break;
				case R.id.rb_fy:
					type = 2;
					scrollView.setVisibility(View.GONE);
					...
					ll_zy_fee.setVisibility(View.VISIBLE);
					break;
				case R.id.rb_bl:
					type = 3;
		...
		reportlist.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1,
						...
						if (arg2 > 0) {
							...
							Intent intent = new Intent(Zl_HosActivity.this,
		...
		case 1:
			if (tempYz == null) {
				...
				ZlYiZhuAdapter yzAdapter = new ZlYiZhuAdapter(
						Zl_HosActivity.this, tempYz);
				lv_items.setAdapter(yzAdapter);
			}
			break;
		case 2:
			if (tempHosCharge == null) {
				getChargeData(pat_no);
			} else {
				ZhuyChargeAdapter yzAdapter = new ZhuyChargeAdapter(
						Zl_HosActivity.this, tempHosCharge.getChargeInfos());
				lv_zy_fee.setAdapter(yzAdapter);
				tv_zy_totle.setText("总费用："+tempHosCharge.getTotleFee()+"（元）");
				tv_zy_time.setText(tempHosCharge.getTimes());
			}
			break;
		case 3:
			if (disHisInfo != null) {
	...
	private void getYzData(String pat_no) {
		...
		gServlet.setOnResponseEndListening(new GsonServlet.OnResponseEndListening<PatYzReq, PtYzRes>() {
			...
			public void onResponseEndSuccess(PatYzReq commonReq,
				...
				if (commonRes != null) {
					ZlYiZhuAdapter yzAdapter = new ZlYiZhuAdapter(
							Zl_HosActivity.this, commonRes.getPtEntities());
					lv_items.setAdapter(yzAdapter);
	...
	private void getChargeData(String pat_no) {
		...
		gServlet.setOnResponseEndListening(new GsonServlet.OnResponseEndListening<HospitalChargeReq, HospitalChargeRes>() {
			...
			public void onResponseEndSuccess(HospitalChargeReq commonReq,
				...
				// TODO Auto-generated method stub
				if (commonRes != null&&commonRes.getChargeInfos()!=null) {
					tempHosCharge=commonRes;
					ZhuyChargeAdapter chargeAdapter=new ZhuyChargeAdapter(Zl_HosActivity.this,commonRes.getChargeInfos());
					lv_zy_fee.setAdapter(chargeAdapter);
					tv_zy_time.setText(commonRes.getTimes());
					tv_zy_totle.setText("总费用："+commonRes.getTotleFee()+"（元）");
				}
			}
	...
	private void bindBs(DiseaseHistoryInfo disinfo) {
		LinearLayout layout = (LinearLayout) LinearLayout.inflate(
				Zl_HosActivity.this, R.layout.zl_jbs_item, null);
				...
				if (commonRes != null) {
					...
					if (reportlistdata != null && reportlistdata.size() > 0) {
						...
						TbReportListAdapter adapter = new TbReportListAdapter(
								Zl_HosActivity.this, reportlistdata,
								preferences);
